...
const CustomSection = ({
  ...
  const icons = [templateIcon, colorPickerIcon, typoIcon];

  const titles = ["템플릿", "색상 팔레트", "타이포그래피"];
  ...
  return (
    <div className="custom-container">
      ...
      <div
        className={`custom-list-wrapper ${isCustomViewOpen ? "active" : ""}`}
      >
        <div className="custom-title-wrapper">
          <p className="custom-title">{customTitle}</p>
          <button className="custom-close-button" onClick={onClickClose}>
            <img
              src={customCloseImage}
              width="20px"
              height="20px"
              alt="close"
            />
